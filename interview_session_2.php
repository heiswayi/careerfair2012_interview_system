...
<?php
include('config.php');
$result = mysql_query("SELECT * FROM $company WHERE type='Interview' AND date='Day 2' ORDER BY name") or die(mysql_error());
$totalsession = mysql_num_rows($result);
echo '<div class="box_title">' . $totalsession . ' company(s) available for <u>Interview Session</u> on <u>Day 2:</u> ' . $day2 . '</div>';
...
while($row = mysql_fetch_array($result)) {
            ...
            $thecompany = $row['name'];
                ...
                echo '<a class="uiButton uiButtonNormal" href="namelist.php?id=' . $row['id'] .'&day=2">' . $totali . ' ' . $totalix . '</a>';
            echo '</td>';
