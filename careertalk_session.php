...
<?php
include('config.php');
$result = mysql_query("SELECT * FROM $company WHERE type='Career Talk' AND date='Day 1' ORDER BY name") or die(mysql_error());
$totalsession = mysql_num_rows($result);
echo '<div class="box_title">' . $totalsession . ' company(s) available for <u>Career Talk Session</u> on <u>Day 1:</u> ' . $day1 . '</div>';
...
while($row = mysql_fetch_array($result)) {
            ...
            $thecompany = $row['name'];
                ...
                echo '<a class="uiButton uiButtonNormal" href="namelist.php?id=' . $row['id'] .'&day=1">' . $totalc . ' ' . $totaliy . '</a>';
            echo '</td>';
